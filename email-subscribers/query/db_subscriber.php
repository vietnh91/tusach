...
class es_cls_dbquery {
	...
	public static function update_donate($data = array()) {
		...
		$result = 0;
		$CurrentDate = date('Y-m-d G:i:s');
		$sql = $wpdb->prepare("INSERT INTO `".$prefix."donate` 

				(`name`, `phone`, `recipient_type`, `recipient_name`, `recipient_address`, `description`, `email`, `create_date`)

				VALUES(%s, %s, %d, %s, %s, %s, %s, %s)", array(trim($data["name"]), trim($data["phone"]), 

				trim($data["recipient_type"]), trim($data["recipient_name"]), trim($data["recipient_address"]), trim($data["description"]), trim($data["email"]), $CurrentDate));
